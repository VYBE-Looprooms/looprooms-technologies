...
router.post("/", waitlistLimiter, async (req, res) => {
  try {
    ...
    console.log("Received waitlist data:", {
      email,
      type,
      name,
      firstName,
    ...
    // Send welcome email
    const emailTemplate =
      type === "creator" ? "waitlistCreator" : "waitlistUser";
    const displayName = parsedFirstName || name || "there";

    console.log(
      `Attempting to send ${emailTemplate} email to ${email} with name: ${displayName}`
    );
    ...
    const stats = await Waitlist.findAll({
      attributes: [
        ...
        [
          sequelize.fn(
            "COUNT",
            sequelize.literal("CASE WHEN type = 'user' THEN 1 END")
          ),
          "users",
        ],
        [
          sequelize.fn(
            "COUNT",
            sequelize.literal("CASE WHEN type = 'creator' THEN 1 END")
          ),
          "creators",
        ],
        [
          sequelize.fn(
            "COUNT",
            sequelize.literal(
              "CASE WHEN created_at >= NOW() - INTERVAL '24 hours' THEN 1 END"
            )
          ),
          "today",
        ],
        [
          sequelize.fn(
            "COUNT",
            sequelize.literal(
              "CASE WHEN created_at >= NOW() - INTERVAL '7 days' THEN 1 END"
            )
          ),
          "thisWeek",
        ],
